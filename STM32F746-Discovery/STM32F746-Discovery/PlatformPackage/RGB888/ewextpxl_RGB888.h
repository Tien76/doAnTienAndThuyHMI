...
*   based on the RGB888 screen and RGBA8888 native pixel format whereby the
...
*
*     The format name 'RGB888' refers to the generic 24 bit color format where
*     the color channels are stored within a 24 bit value in the following
*     manner:
*
*              23           16             8            0
*               +---------------------------------------+
...
*     are available (XXX stands for RED, GREEN and BLUE). For example to adapt
*     this pixel driver to graphics hardware using BGR888 color format, add 
*     following lines to your 'ewextgfx.h' file:
*
*       #define EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_RED   16
*       #define EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_BLUE  0
*
*     Please note, these options affect the screen pixel format RGB888 only.
*
*   > Custom specific color channel order of the RGB565 pixel format. Depending
*     on the graphics hardware, it may be necessary to adapt the order in which
*     the color channels are stored in the video memory of an RGB565 surface.
*
...
*
*     To control the order of channels, specify the bit start position of each
*     channel. For this purpose the macros EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_XXX
...
/* If not explicitly defined, assume default bit positions for the RGB channels
  ...
  #define EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_BLUE  16
#endif


/* Verify the position of the red, green, blue and alpha channel */
#if (( EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_RED != 0  ) &&                       \
     ( EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_RED != 8  ) &&                       \
     ( EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_RED != 16 ))
  #error Wrong 'red' channel bit position. Only '0', '8' or '16' allowed.
#endif

#if (( EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_GREEN != 0  ) &&                     \
     ( EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_GREEN != 8  ) &&                     \
     ( EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_GREEN != 16 ))
  #error Wrong 'green' channel bit position. Only '0', '8' or '16' allowed.
#endif

#if (( EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_BLUE != 0  ) &&                      \
     ( EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_BLUE != 8  ) &&                      \
     ( EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_BLUE != 16 ))
  #error Wrong 'blue' channel bit position. Only '0', '8' or '16' allowed.
#endif
...
/* If not explicitly defined, assume default bit positions for the RGB channels
   of the RGB565 format within a pixel memory. */
#ifndef EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_RED
  #define EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_RED    0
#endif
  ...
  #define EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_BLUE  11
#endif


/* Verify the position of the red, green and blue channels of the RGB565 
   format */
#if (( EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_RED != 0  ) &&                       \
     ( EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_RED != 11 ))
  #error Wrong 'red' channel bit position. Only '0' or '11' allowed.
#endif

#if EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_GREEN != 5
  #error Wrong 'green' channel bit position. Only '5' allowed.
#endif

#if (( EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_BLUE != 0  ) &&                      \
     ( EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_BLUE != 11 ))
  #error Wrong 'blue' channel bit position. Only '0' or '11' allowed.
#endif

#if EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_RED ==                                  \
    EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_BLUE
  #error Bit offset conflict in the color channel definition.
#endif


/* The following mode can be enabled for Index8 target systems only. RGB888 / 
...
/* RGB888 / RGBA8888 expects the 'screen' color format to be enabled in the
